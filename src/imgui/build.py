...
backends = {
  ...
  "sdl2":         { "supported": False,  "deps": ["sdl2"] },
  "sdl3":         { "supported": True,  "deps": ["sdl3"] },
  "sdlrenderer2": { "supported": False,  "deps": ["sdl2"] },
  "sdlrenderer3": { "supported": False },
  "vulkan":       { "supported": False,  "defines": ["VK_NO_PROTOTYPES"], "deps": ["vulkan"] },
  "wgpu":         { "supported": False,  "deps": ["wgpu"] },
...
backend_deps = {
  ...
  "sdl3":   { "repo": "https://github.com/libsdl-org/SDL.git",               "commit": "release-3.2.4",  "path": "SDL3" },
...
def exec_vcvars(cmd: typing.List[str], what):
  ...
  if len(what) > max_what_len:
    what = what[:max_what_len - 2] + ".."
  print(what + (" " * (max_what_len - len(what))) + "> " + " ".join(cmd))
  assertx(subprocess.run(' '.join(cmd), shell=True).returncode == 0, f"Failed to run command '{cmd}'")
